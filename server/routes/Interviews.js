...
router.route("/displayci").get((req,res)=>{
    //body
    Interviews.find({QuestionType: 'Common Interview Question'}).then((questions)=>{
        res.json(questions)
    }).catch((err)=>{
        console.log(err);
    })
...
router.route("/displayig").get((req,res)=>{
    //body
    Interviews.find({QuestionType: 'Interview Guidelines'}).then((questions)=>{
        res.json(questions)
    }).catch((err)=>{
        console.log(err);
    })
...
router.route("/displaysq").get((req,res)=>{
    //body
    Interviews.find({SaveOp: 'yes'}).then((questions)=>{
        res.json(questions)
    }).catch((err)=>{
        console.log(err);
    })
...
router.route("/displayuserqp").get((req,res)=>{
    //body
    Interviews.find({User: 'Anuradha'}).then((questions)=>{
        res.json(questions)
    }).catch((err)=>{
        console.log(err);
    })
...
router.route("/update/:id").put(async(req,res)=>{
    ...
    const update = await Interviews.findByIdAndUpdate(qno,updateInterview).then(()=>{
...
router.route("/delete/:id").delete(async(req,res) => {
    ...
    await Interviews.findByIdAndDelete(qno).then(()=>{
